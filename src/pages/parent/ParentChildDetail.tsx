...
const ParentChildDetail: React.FC = () => {
  ...
  const { data: report, isLoading } = useQuery({
    queryKey: ['child-grades-report', id, academicYear],
    queryFn: () =>
      gradesApi
        .report({ student_id: Number(id), academic_year: academicYear })
        .then((r) => r.data.data),
    enabled: !!id,
  });
  ...
  const studentName = report?.student.full_name ?? `Élève #${id}`;

  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader
        title={studentName}
        subtitle="Bulletin de notes"
        breadcrumbs={[
          ...
          <div className="bg-school-900 rounded-xl p-6 text-white flex items-center justify-between shadow-card">
            <div>
              ...
              <p className="text-white/50 text-xs mt-2">
                {report.student.full_name} · {academicYear}
              </p>
            </div>
              ...
              <table className="w-full">
                ...
                <tfoot>
                  <tr className="bg-school-900">
                    <td colSpan={3} className="px-5 py-3 text-white font-semibold text-sm">
                      Moyenne générale
                    </td>
