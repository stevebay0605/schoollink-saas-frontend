...
import { BookMarked, ClipboardList, Calendar, BookOpen } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        subtitle="Bienvenue dans votre espace enseignant"
      />
        ...
        {isLoading ? (
          ...
          <>
            ...
            <StatCard
              ...
              value={stats?.my_courses?.length ?? 0}
              icon={<Calendar size={20} />}
              color="secondary"
            />
            <StatCard
              title="Devoirs ouverts"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-border p-5 shadow-card">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg bg-school-50 flex items-center justify-center">
              <BookMarked size={15} className="text-school-600" />
            </div>
            <h2 className="font-semibold text-foreground text-base">Mes cours récents</h2>
          </div>

          {isLoading ? (
            <Skeleton lines={5} />
          ) : !stats?.my_courses?.length ? (
            ...
            <ul className="space-y-2">
              {stats.my_courses.slice(0, 5).map((course) => (
                <li
                  key={course.id}
        ...
        <div className="bg-white rounded-xl border border-border p-5 shadow-card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-semibold text-foreground text-base">Devoirs en cours</h2>
          </div>

          {isLoading ? (
            <Skeleton lines={5} />
          ) : !stats?.my_assignments?.filter((a) => a.status === 'open').length ? (
            <div className="py-8 text-center">
              <ClipboardList size={32} className="mx-auto text-muted-foreground/30 mb-2" />
              <p className="text-sm text-muted-foreground">Aucun devoir actif</p>
            </div>
          ) : (
            <ul className="space-y-2">
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground truncate">
                    {subject.classroom?.name ?? '—'} · Coeff.&nbsp;{subject.coefficient}
                  </div>
                </div>
              </div>
            ))}
